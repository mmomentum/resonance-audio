...
namespace vraudio {
	...
	void ReflectionsProcessor::Update(
		...
		const float low_pass_coefficient = ComputeLowPassMonoPoleCoefficient(reflection_properties.cutoff_frequency, sample_rate_);
		...
		GetRelativeDirection(WorldPosition(reflection_properties.room_position), WorldRotation(reflection_properties.room_rotation).conjugate(), listener_position, &relative_listener_position);

		ComputeReflections(relative_listener_position, WorldPosition(reflection_properties.room_dimensions), reflection_properties.coefficients, &target_reflections_);

		// Additional |frames_per_buffer_| to process needed to compensate the
		// crossfade between the current and target reflections.
		num_frames_to_process_on_empty_input_ = frames_per_buffer_ + static_cast<size_t>(FindMaxReflectionDelayTime(target_reflections_) * static_cast<float>(sample_rate_));
